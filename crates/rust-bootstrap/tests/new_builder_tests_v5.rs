...
mod tests {
    ...
    #[test]
    fn test_builder_initialization_v5() {
    ...
    #[test]
    fn test_builder_bootstrap_binary_path_v5() {
        let args = Args::parse_from(vec!["rust-bootstrap"]);
        let rust_root = PathBuf::from("target/new_test_builder_v5/rust_root");
        let build_dir = PathBuf::from("target/new_test_builder_v5/build_dir");
        let stage0 = Stage0 {
            rustc: PathBuf::from("/dummy/rustc"),
            cargo: PathBuf::from("/dummy/cargo"),
            compiler_date: "2025-08-30".to_string(),
