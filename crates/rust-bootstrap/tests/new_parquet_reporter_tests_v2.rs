...
mod tests {
    ...
    #[test]
    fn test_write_build_config_to_parquet_v2() {
        let temp_dir = PathBuf::from("target/new_test_parquet_reporter_v2/build_config");
        if temp_dir.exists() {
    ...
    #[test]
    fn test_write_record_batch_to_parquet_v2() {
        let temp_dir = PathBuf::from("target/new_test_parquet_reporter_v2/record_batch");
        if temp_dir.exists() {
            fs::remove_dir_all(&temp_dir).unwrap();
        }
        fs::create_dir_all(&temp_dir).unwrap();
    ...
    #[test]
    fn test_read_and_summarize_build_config_metrics_v2() {
        // This test requires a valid parquet file to read from.
        // We can reuse the file created by test_write_build_config_to_parquet.
        let temp_dir = PathBuf::from("target/new_test_parquet_reporter_v2/summarize");
        if temp_dir.exists() {
            fs::remove_dir_all(&temp_dir).unwrap();
        }
        fs::create_dir_all(&temp_dir).unwrap();
