...
mod tests {
    ...
    #[test]
    fn test_write_build_config_to_parquet() {
        let temp_dir = PathBuf::from("target/test_parquet_reporter/build_config");
        if temp_dir.exists() {
    ...
    #[test]
    fn test_write_record_batch_to_parquet() {
        let temp_dir = PathBuf::from("target/test_parquet_reporter/record_batch");
        if temp_dir.exists() {
            fs::remove_dir_all(&temp_dir).unwrap();
        }
        fs::create_dir_all(&temp_dir).unwrap();
