...
mod tests {
    ...
    #[test]
    fn test_builder_initialization_v3() {
    ...
    #[test]
    fn test_builder_bootstrap_binary_path_v3() {
        let args = Args::parse_from(vec!["rust-bootstrap"]);
        let rust_root = PathBuf::from("target/new_test_builder_v3/rust_root");
        let build_dir = PathBuf::from("target/new_test_builder_v3/build_dir");
        let stage0 = Stage0 {
            rustc: PathBuf::from("/dummy/rustc"),
            cargo: PathBuf::from("/dummy/cargo"),
            compiler_date: "2025-08-30".to_string(),
