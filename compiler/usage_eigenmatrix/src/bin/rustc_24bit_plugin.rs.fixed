...
fn collect_defids_to_matrix(tcx: TyCtxt<'_>) {
    ...
    if matrix.is_empty() {
        *matrix = vec![None; 4096]; // Initialize 4096 pages
    }
    
    let mut count = SIGNATURE_COUNT.lock().unwrap();
    
    // Get all DefIds from the crate
    let local_def_ids = tcx.hir().body_owners();
...
fn map_defid_to_24bit(def_id: DefId) -> u32 {
    ...
    // Combine crate and index into 24-bit signature
...
fn main() {
    ...
    let result = rustc_driver::RunCompiler::new(&rustc_args, &mut callbacks).run();
