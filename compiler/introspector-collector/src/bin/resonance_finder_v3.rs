use std::collections::HashMap;
use std::fs;
use serde_json::Value;

#[derive(Debug, Clone)]
struct ResonanceNode {
    name: String,
    complexity: f64,
    usage_count: usize,
    resonance_score: f64,
}

fn calculate_resonance(node1: &str, node2: &str, usage1: usize, usage2: usize) -> f64 {
    let name_similarity = jaccard_similarity(node1, node2);
    let usage_ratio = (usage1.min(usage2) as f64) / (usage1.max(usage2) as f64).max(1.0);
    name_similarity * usage_ratio * 100.0
}

fn jaccard_similarity(s1: &str, s2: &str) -> f64 {
    let chars1: std::collections::HashSet<char> = s1.chars().collect();
    let chars2: std::collections::HashSet<char> = s2.chars().collect();
    let intersection = chars1.intersection(&chars2).count();
    let union = chars1.union(&chars2).count();
    if union == 0 { 0.0 } else { intersection as f64 / union as f64 }
}

fn load_comprehensive_usage_data() -> HashMap<String, (usize, f64)> {
    let mut usage_map = HashMap::new();
    
    // Load from comprehensive usage report
    if let Ok(content) = fs::read_to_string("./comprehensive_usage_report.json") {
        if let Ok(json) = serde_json::from_str::<Value>(&content) {
            if let Some(crate_stats) = json["crate_stats"].as_object() {
                for (crate_name, stats) in crate_stats {
                    let mut total_usage = 0;
                    let mut complexity = 0.0;
                    
                    if let Some(stats_obj) = stats.as_object() {
                        for (_, count) in stats_obj {
                            if let Some(num) = count.as_u64() {
                                total_usage += num as usize;
                                complexity += num as f64;
                            }
                        }
                    }
                    
                    usage_map.insert(crate_name.clone(), (total_usage, complexity));
                }
            }
        }
    }
    
    usage_map
}

fn find_resonant_nodes(target_nodes: &[&str]) -> HashMap<String, Vec<ResonanceNode>> {
    let mut resonances = HashMap::new();
    let usage_data = load_comprehensive_usage_data();
    
    for &target in target_nodes {
        let mut resonant_nodes = Vec::new();
        
        for (crate_name, (usage_count, complexity)) in &usage_data {
            let score = calculate_resonance(target, crate_name, 100, *usage_count);
            if score > 5.0 {
                resonant_nodes.push(ResonanceNode {
                    name: crate_name.clone(),
                    complexity: *complexity,
                    usage_count: *usage_count,
                    resonance_score: score,
                });
            }
        }
        
        resonant_nodes.sort_by(|a, b| b.resonance_score.partial_cmp(&a.resonance_score).unwrap());
        resonant_nodes.truncate(5);
        resonances.insert(target.to_string(), resonant_nodes);
    }
    
    resonances
}

fn main() {
    println!("🌊 Resonance Finder: Discovering Mathematical Harmonics in Rust Ecosystem");
    
    let target_nodes = ["TypeNode", "FunctionArrow", "EnumOrbit", "RustLanguage", "usage_ratio", "main"];
    
    let resonances = find_resonant_nodes(&target_nodes);
    
    for (target, nodes) in &resonances {
        println!("\n🎯 Resonances for {}: ", target);
        if nodes.is_empty() {
            println!("  No strong resonances found (threshold: 5.0)");
        } else {
            for (i, node) in nodes.iter().enumerate() {
                println!("  {}. {} (score: {:.2}, usage: {}, complexity: {:.1})", 
                    i + 1, node.name, node.resonance_score, node.usage_count, node.complexity);
            }
        }
    }
    
    let total_energy: f64 = resonances.values()
        .flat_map(|nodes| nodes.iter())
        .map(|node| node.resonance_score)
        .sum();
    
    println!("\n✨ Total Resonance Energy: {:.2}", total_energy);
    
    // Show some high-usage crates for context
    let usage_data = load_comprehensive_usage_data();
    let mut high_usage: Vec<_> = usage_data.iter().collect();
    high_usage.sort_by(|a, b| b.1.0.cmp(&a.1.0));
    
    println!("\n📊 Top usage patterns in ecosystem:");
    for (name, (usage, _)) in high_usage.iter().take(5) {
        println!("  • {}: {} usages", name, usage);
    }
    
    println!("🔮 Mathematical harmonics analysis complete");
}
