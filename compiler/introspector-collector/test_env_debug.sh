...
USAGE_OUTPUT_DIR="./test_usage_data" \
RUSTC="$(pwd)/collector_store/working_usage_collector" \
cargo build -p rustc_driver 2>&1 | tee -a build_with_collector.log

echo "🧠 === Phase 3: Core Compiler Components ==="
# Build essential compiler crates
USAGE_OUTPUT_DIR="./test_usage_data" \
RUSTC="$(pwd)/collector_store/working_usage_collector" \
cargo build -p rustc_middle -p rustc_hir -p rustc_ast 2>&1 | tee -a build_with_collector.log

echo "🎯 === Phase 4: Analysis Tools ==="
# Build our analysis binaries
USAGE_OUTPUT_DIR="./test_usage_data" \
RUSTC="$(pwd)/collector_store/working_usage_collector" \
cargo build --bin embedded_graph_advisor --bin self_improving_driver --bin enhanced_dwim_corrector 2>&1 | tee -a build_with_collector.log
