...
fn main() {
    ...
    let eigenmatrix_content = fs::read_to_string("rust_eigenmatrix.json")
        .expect("Failed to read eigenmatrix file");
    
    let eigenmatrix: Value = serde_json::from_str(&eigenmatrix_content)
    ...
    let mut feature_weights: HashMap<String, f64> = features.iter()
    ...
    for (parent, child, weight) in &arrows {
        ...
        relationship_types.entry(child_type.to_string())
            .or_insert_with(Vec::new)
            .push((parent.clone(), child.clone(), *weight));
    }
    ...
    for (rel_type, relationships) in sorted_types.iter().take(10) {
        ...
        println!("{:15} | Count: {:2} | Total Weight: {:.2e}", 
                 rel_type, relationships.len(), total_weight);
    ...
    let max_depth = features.iter()
        .map(|f| f.as_str().unwrap().matches('.').count())
        .max().unwrap_or(0);
    
    println!("\n🌊 Deepest AST Paths (Depth {}):", max_depth);
    let deep_paths: Vec<&str> = features.iter()
