...
impl HierarchicalSignatureExtractor {
    pub fn new() -> Self {
        Self {
            ...
            level_transitions: HashMap::new(),
        }
    }
    
    pub fn extract_all_levels(&mut self, file_path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn extract_byte_signature(&self, file_path: &str) -> Result<LevelSignature, Box<dyn std::error::Error>> {
        ...
        let frequency_signature: Vec<f64> = byte_freq.iter()
            .map(|&count| count as f64 / content.len() as f64)
            .collect();
        ...
        features.insert("ascii_ratio".to_string(), content.iter().filter(|&&b| b < 128).count() as f64 / content.len() as f64);
    ...
    fn extract_syn_signature(&self, file_path: &str, byte_sig: &LevelSignature) -> Result<LevelSignature, Box<dyn std::error::Error>> {
        ...
        features.insert("avg_token_length".to_string(), tokens.iter().map(|t| t.len()).sum::<usize>() as f64 / tokens.len() as f64);
    ...
    fn extract_span_signature(&self, file_path: &str, syn_sig: &LevelSignature) -> Result<LevelSignature, Box<dyn std::error::Error>> {
        let content = std::fs::read_to_string(file_path)?;
        ...
        span_features.insert("avg_line_length".to_string(), lines.iter().map(|l| l.len()).sum::<usize>() as f64 / lines.len() as f64);
        span_features.insert("max_line_length".to_string(), lines.iter().map(|l| l.len()).max().unwrap_or(0) as f64);
        ...
        let frequency_signature: Vec<f64> = line_length_dist.iter()
            .map(|&count| count as f64 / lines.len() as f64)
            .collect();
    ...
    fn extract_hir_signature(&self, _file_path: &str, span_sig: &LevelSignature) -> Result<LevelSignature, Box<dyn std::error::Error>> {
        // Mock HIR analysis - would use rustc_hir
        let mut hir_features = HashMap::new();
        hir_features.insert("estimated_nodes".to_string(), span_sig.structural_features.get("line_count").unwrap_or(&0.0) * 2.5);
        hir_features.insert("control_flow_complexity".to_string(), span_sig.structural_features.get("line_count").unwrap_or(&0.0) * 0.1);
    ...
    fn extract_ty_signature(&self, _file_path: &str, hir_sig: &LevelSignature) -> Result<LevelSignature, Box<dyn std::error::Error>> {
        // Mock type analysis - would use rustc_middle::ty
        let mut ty_features = HashMap::new();
        ty_features.insert("estimated_types".to_string(), hir_sig.structural_features.get("estimated_nodes").unwrap_or(&0.0) * 0.3);
        ty_features.insert("generic_complexity".to_string(), hir_sig.structural_features.get("control_flow_complexity").unwrap_or(&0.0) * 1.5);
        ty_features.insert("trait_bounds".to_string(), hir_sig.structural_features.get("estimated_nodes").unwrap_or(&0.0) * 0.1);
    ...
    fn record_level_transitions(&mut self, signatures: &[LevelSignature]) {
        for i in 0..signatures.len()-1 {
            ...
            let transition: Vec<f64> = signatures[i].frequency_signature.iter()
                .zip(&signatures[i+1].frequency_signature)
                .map(|(a, b)| b - a)
                .collect();
            
            self.level_transitions.entry((from_level, to_level))
