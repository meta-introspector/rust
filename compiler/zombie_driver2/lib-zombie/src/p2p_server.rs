...
use crate::plugin_driver::{PluginDriver, CompilerEvent};
...
impl P2PPluginServer {
    ...
    fn parse_verb(&self, msg: &str) -> Result<P2PVerb, Box<dyn std::error::Error>> {
        ...
        match parts[0] {
            ...
            "REGISTER" => Ok(P2PVerb::RegisterEvent(parts[1].to_string(), parts[2].parse()?)),
            "ATTACH" => Ok(P2PVerb::AttachData(parts[1].to_string(), parts[2].as_bytes().to_vec())),
            "RUN_FILES" => Ok(P2PVerb::RunWithFiles(parts[1].to_string(), parts[2..].iter().map(|s| s.to_string()).collect())),
            "CAPTURE" => Ok(P2PVerb::CaptureResult(parts[1].to_string())),
            "COMPILE_SRC" => Ok(P2PVerb::CompileSource(parts[1].to_string(), parts[2..].join(" "))),
            "COMPILE_FILE" => Ok(P2PVerb::CompileFile(parts[1].to_string(), parts[2].to_string())),
            "INVOKE" => Ok(P2PVerb::InvokeFunction(parts[1].to_string(), parts[2].to_string(), parts.get(3).unwrap_or(&"0").parse().unwrap_or(0))),
