...
impl ModuleSpectralProfiler {
    pub fn new() -> Self {
        ...
        Self { modules, flags, samples: Vec::new(), analyses: Vec::new() }
    }
    ...
    fn profile_module_with_flag(
        &mut self,
        module: &str,
        flag: &str,
    ) -> Result<(), Box<dyn std::error::Error>> {
        let perf_data =
            format!("perf_{}_{}.data", module, flag.replace("--", "").replace("=", "_"));
    ...
    fn analyze_spectrum(
        ...
        let module_samples: Vec<&PerfSample> =
            self.samples.iter().filter(|s| s.module == module && s.flag == flag).collect();
        ...
        for sample in module_samples {
            ...
            if sample.function.contains("enum") || sample.function.contains("variant") {
                ...
                enum_cycles.entry(enum_name).or_insert_with(Vec::new).push(sample.cycles);
            }
        }
        ...
        let all_spectra: Vec<f64> = function_frequencies
            .values()
        ...
        println!(
            "📊 Spectral analysis complete: {} {} (dominant: {:.2} Hz)",
            module, flag, dominant_frequency
        );
    ...
    fn classify_spectrum(
        ...
        let label = match category {
            LabelCategory::Function => {
                if dominant_freq > 10.0 {
                    format!("hot_func_{}", name)
                } else if periodicity > 0.8 {
                    format!("periodic_func_{}", name)
                } else {
                    format!("cold_func_{}", name)
                }
            }
            LabelCategory::Enum => {
                if entropy > 0.7 {
                    format!("variant_heavy_enum_{}", name)
                } else {
                    format!("simple_enum_{}", name)
                }
            }
            LabelCategory::MemoryAddress => {
                if dominant_freq > 20.0 {
                    format!("hot_memory_{}", name)
                } else {
                    format!("cold_memory_{}", name)
                }
            }
            LabelCategory::File => {
                if periodicity > 0.9 {
                    format!("streaming_file_{}", name)
                } else {
                    format!("random_access_file_{}", name)
                }
            }
            LabelCategory::TypeField => {
                if dominant_freq > 5.0 {
                    format!("active_field_{}", name)
                } else {
                    format!("passive_field_{}", name)
                }
            }
            LabelCategory::SystemCall => {
                if periodicity > 0.8 {
                    format!("regular_syscall_{}", name)
                } else {
                    format!("burst_syscall_{}", name)
                }
            }
    ...
    fn convert_to_spectra(&self, data: HashMap<String, Vec<u64>>) -> HashMap<String, Vec<f64>> {
        data.into_iter().map(|(k, v)| (k, self.compute_fft(&v))).collect()
    }
