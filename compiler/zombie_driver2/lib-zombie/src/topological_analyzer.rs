...
impl TopologicalCompilationAnalyzer {
    pub fn new() -> Self {
        Self { morse_points: Vec::new(), k_classes: Vec::new(), quasi_fibrations: Vec::new() }
    }
    ...
    fn compute_chern_character(&self, data: &[f64]) -> Vec<f64> {
        ...
        data.iter().enumerate().map(|(i, &x)| x * (i as f64).exp()).collect()
    }
    ...
    pub fn bott_periodicity_theorem(&self) -> String {
        ...
        let complex_classes: Vec<_> = self
            .k_classes
            .iter()
            .filter_map(|k| match &k.periodicity_class {
