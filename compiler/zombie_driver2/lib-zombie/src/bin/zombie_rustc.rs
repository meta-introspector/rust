...
fn main() {
    ...
    for file in &rust_files {
        ...
        let file_stem = Path::new(file).file_stem().and_then(|s| s.to_str()).unwrap_or("unknown");
                    ...
                    match serde_json::to_string_pretty(&analysis) {
                        Ok(json) => {
                            ...
                            match std::fs::write(&syn_output, json) {
                                Ok(_) => println!(
                                    "📁 Syn analysis: {}",
                                    std::fs::canonicalize(&syn_output)
                                        .unwrap_or_else(|_| syn_output.into())
                                        .display()
                                ),
    ...
    println!(
        "📁 Combined analysis: {}",
        std::fs::canonicalize(&combined_output)
            .unwrap_or_else(|_| combined_output.clone().into())
            .display()
    );
    ...
    println!(
        "   Combined analysis: {}",
        std::fs::canonicalize(&combined_output)
            .unwrap_or_else(|_| combined_output.into())
            .display()
    );
    ...
    for emit_type in &emit_types {
        match emit_type.as_str() {
            "dep-info" => {
                ...
                let _ = std::fs::write(
                    &dep_file,
                    format!("# Zombie rustc dep-info for {}\n", crate_name),
                );
            }
            "metadata" => {
            ...
            "link" => {
                ...
                if args.iter().any(|arg| {
                    arg == "--crate-type"
                        && args.get(args.iter().position(|x| x == arg).unwrap() + 1)
                            == Some(&"bin".to_string())
                }) {
                    let bin_file = format!("{}/{}{}", output_dir, crate_name, extra_filename);
                    let _ = std::fs::write(&bin_file, b"#!/bin/bash\necho 'Zombie binary stub'\n");
                    let _ =
                        std::fs::set_permissions(&bin_file, std::fs::Permissions::from_mode(0o755));
                } else {
                    let lib_file =
                        format!("{}/lib{}{}.rlib", output_dir, crate_name, extra_filename);
