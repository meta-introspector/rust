...
fn verify_so_proof(so_path: &str, proof: &SOProof) -> Result<bool, Box<dyn std::error::Error>> {
    ...
    // Verify signature
    let public_key = VerifyingKey::from_bytes(&proof.public_key.clone().try_into().map_err(|_| "Invalid public key")?)?;
    let signature = Signature::from_bytes(&proof.signature.clone().try_into().map_err(|_| "Invalid signature")?);
...
fn load_verified_so(so_path: &str, manifest: &SOManifest) -> Result<(), Box<dyn std::error::Error>> {
