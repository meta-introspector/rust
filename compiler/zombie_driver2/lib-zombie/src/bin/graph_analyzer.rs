...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let analysis_file = std::env::args().nth(1)
...
fn build_character_graph(arrows: &[EssentialArrow]) -> Result<(UnGraph<char, f64>, HashMap<char, NodeIndex>, DMatrix<f64>), Box<dyn std::error::Error>> {
    ...
    for arrow in arrows {
        if let (Some(&from_idx), Some(&to_idx)) = (
            char_map.get(&arrow.pair.0),
            char_map.get(&arrow.pair.1),
        ) {
    ...
    let char_to_index: HashMap<char, usize> = chars.iter().enumerate()
        .map(|(i, &ch)| (ch, i))
        .collect();
    
    for arrow in arrows {
        if let (Some(&i), Some(&j)) = (
            char_to_index.get(&arrow.pair.0),
            char_to_index.get(&arrow.pair.1),
        ) {
...
fn generate_dot_graph(graph: &UnGraph<char, f64>, char_map: &HashMap<char, NodeIndex>, arrows: &[EssentialArrow]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (&ch, &node_idx) in char_map {
        ...
        dot.push_str(&format!("  {} [label=\"{}\", fillcolor={}];\n", 
                             node_idx.index(), label, color));
    }
    
    // Add edges with weights
    for arrow in arrows.iter().take(20) { // Top 20 arrows only
        if let (Some(&from_idx), Some(&to_idx)) = (
            char_map.get(&arrow.pair.0),
            char_map.get(&arrow.pair.1),
        ) {
            let thickness = (arrow.strength * 3.0).max(0.5).min(5.0);
            dot.push_str(&format!("  {} -- {} [label=\"{:.2}\", penwidth={}];\n",
                                 from_idx.index(), to_idx.index(), arrow.strength, thickness));
        }
    }
...
fn calculate_eigenmatrix(matrix: &DMatrix<f64>, char_map: &HashMap<char, NodeIndex>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut eigenvalue_pairs: Vec<(usize, f64)> = eigenvalues.iter()
        .enumerate()
        .map(|(i, &val)| (i, val))
        .collect();
    ...
    if let Some((dominant_idx, dominant_val)) = eigenvalue_pairs.first() {
        ...
        let char_to_index: HashMap<char, usize> = char_map.iter()
            .enumerate()
            .map(|(i, (&ch, _))| (ch, i))
            .collect();
        
        let mut char_components: Vec<(char, f64)> = char_to_index.iter()
            .map(|(&ch, &idx)| (ch, dominant_eigenvector[idx]))
            .collect();
