...
#[no_mangle]
pub extern "C" fn analyze_rust_file(
    file_path: *const u8,
    file_path_len: usize,
    content: *const u8,
    content_len: usize,
    ...
    let file_path = unsafe {
        std::str::from_utf8_unchecked(std::slice::from_raw_parts(file_path, file_path_len))
    };
    let content =
        unsafe { std::str::from_utf8_unchecked(std::slice::from_raw_parts(content, content_len)) };
...
pub fn analyze_file(
    file_path: &str,
    content: &str,
) -> Result<SynAnalysis, Box<dyn std::error::Error>> {
        ...
        *path_matrix.entry(path.clone()).or_insert(0) += 1;
        ...
        for part in parts {
            if part.starts_with("Item")
                || part.starts_with("Expr")
                || part.starts_with("Stmt")
                || part.starts_with("Pat")
                || part.starts_with("Type")
                || part.starts_with("Lit")
                || part.starts_with("Vis")
                || part.starts_with("Field")
                || part.starts_with("Arm")
                || part.starts_with("Block")
            {
