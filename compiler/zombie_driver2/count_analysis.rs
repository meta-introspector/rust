...
fn main() {
    ...
    for (i, (file, objects, paths)) in file_stats.iter().take(15).enumerate() {
        println!("{:2}. {:40} | Objects: {:6} | Paths: {:4}", 
                 i+1, 
    ...
    let large_files = file_stats.iter().filter(|(_, objects, _)| *objects > avg_objects_per_file as usize * 2).count();
    let small_files = file_stats.iter().filter(|(_, objects, _)| *objects < avg_objects_per_file as usize / 2).count();
