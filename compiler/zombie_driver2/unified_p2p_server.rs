...
impl UnifiedP2PServer {
        ...
        match verb {
            ...
            P2PVerb::LoadMultipleSo(so_list) => {
                ...
                for (name, path) in so_list {
                    match self.load_so(&name, &path) {
                        Ok(symbol_count) => {
                            results.push(format!("✅ {}: {} symbols", name, symbol_count))
                        }
            ...
            P2PVerb::ListLoadedSo => {
                let loaded: Vec<String> = self
                    ...
                    .iter()
                    .map(|(name, so)| {
                        format!("{}: {} symbols ({})", name, so.symbols.len(), so.path)
                    })
                    .collect();
            ...
            P2PVerb::CompareEnergy(func1, func2) => {
                ...
                let comparison = if energy1 > energy2 {
                    "higher"
                ...
                Ok(format!(
                    "⚡ {} has {} energy than {} ({} vs {})",
                    func1, comparison, func2, energy1, energy2
                ))
            }
    ...
    async fn compile_to_so(
        ...
        let (errors, warnings): (Vec<String>, Vec<String>) =
            stderr.lines().map(|line| line.to_string()).partition(|line| line.contains("error"));
        ...
        if let Some(loaded_so) = self.loaded_sos.get(so_name) {
            unsafe {
                ...
                let func: Symbol<unsafe extern "C" fn() -> i32> =
                    loaded_so.library.get(func_name.as_bytes())?;
                let result = func();
    ...
    async fn generate_parquet_dataset(
        &self,
        _dataset_name: &str,
        output_path: &str,
    ) -> Result<String, Box<dyn std::error::Error>> {
    ...
    async fn reboot_self(&self) -> Result<String, Box<dyn std::error::Error>> {
        ...
        tokio::process::Command::new(&current_exe).args(&args[1..]).spawn()?;
    ...
    fn get_self_status(&self) -> String {
        format!(
            "Loaded SOs: {}, Rustc: {}, Cargo: {}, Functions: {}",
            self.loaded_sos.len(),
    ...
    fn call_rustc_main(&self, args: &[String]) -> Result<i32, Box<dyn std::error::Error>> {
        ...
        if let Some(rustc_so) = self.loaded_sos.get("rustc_driver") {
            unsafe {
                ...
                if let Ok(main_func) =
                    rustc_so.library.get::<unsafe extern "C" fn(i32, *const *const i8) -> i32>(
                        b"_ZN17rustc_driver_impl4main17hae25326fb9b31672E",
                    )
                {
                    println!("🎯 Found actual rustc_driver_impl::main!");

                    let c_args: Vec<std::ffi::CString> =
                        args.iter().map(|s| std::ffi::CString::new(s.as_str()).unwrap()).collect();
    ...
    fn find_call_only_functions(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Some(rustc_so) = self.loaded_sos.get("rustc_driver") {
            ...
            let wrapper_patterns = ["run_compiler", "interface", "driver", "compile", "main"];
    ...
    async fn compile_via_rustc(
        ...
        let result = CompilationResult {
            ...
            errors: if success {
                ...
                vec![format!("rustc exited with code {}", exit_code)]
            },
            warnings: vec![],
        };
                ...
                if let Some(cargo_so) = self.loaded_sos.get("cargo") {
                    unsafe {
                        ...
                        if let Ok(main_func) =
                            cargo_so
                                .library
                                .get::<unsafe extern "C" fn(i32, *const *const i8) -> i32>(b"main")
                        {
...
async fn async_main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let peer = PeerInfo {
        ...
        mathematical_capabilities: vec![
            "lattice_analysis".to_string(),
            "parquet_generation".to_string(),
        ],
...
"#;
    ...
    let result = server
        .execute_verb(P2PVerb::CompileViaRustc(
