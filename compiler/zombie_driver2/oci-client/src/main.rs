...
impl OciClient {
    ...
    pub async fn get_instance_configuration(&self, config_id: &str) -> Result<InstanceConfiguration> {
        let url = format!(
...
async fn main() -> Result<()> {
    ...
    match client.get_instance_configuration(config_id).await {
        Ok(config) => {
            ...
            println!("  Image: {}", config.instance_details.launch_details.source_details.image_id);
